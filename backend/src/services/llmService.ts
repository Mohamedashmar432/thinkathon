...
import { aiGateway, generateAIResponse } from './ai/aiGateway';
...
class LLMService {
  ...
  async generateRecommendations(inventory: UserInventory): Promise<SecurityRecommendation[]> {
    // Check cache first
  ...
  async generateGeminiRecommendations(request: RecommendationRequest): Promise<any[]> {
    try {
...
`;
      ...
      const validRecommendations = recommendations.filter((rec: any) => 
        rec.title && rec.description && rec.action && rec.whyItMatters
      ).map((rec: any) => ({
   ...
   */
  private async generateLegacyGeminiRecommendations(request: RecommendationRequest): Promise<any[]> {
    try {
  ...
  private generateCacheKey(inventory: UserInventory): string {
  ...
  private async getCachedRecommendations(cacheKey: string): Promise<SecurityRecommendation[] | null> {
    try {
      const cached = await LLMCache.findOne({ 
        cacheKey,
  ...
  private async cacheRecommendations(cacheKey: string, recommendations: SecurityRecommendation[]): Promise<void> {
    try {
  ...
  private buildRecommendationPrompt(inventory: UserInventory): string {
    return `
...
Generate 3-5 recommendations focusing on the highest impact actions.`;
  ...
  private parseRecommendations(text: string, inventory: UserInventory): SecurityRecommendation[] {
    try {
  ...
  private getDefaultRecommendations(inventory: UserInventory): SecurityRecommendation[] {
...
`;
  ...
  private getDefaultScoreExplanation(inventory: { secureScore: number }): string {
...
export async function generateGeminiRecommendations(request: RecommendationRequest): Promise<any[]> {
