...
router.get('/', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const latestScans = await Scan.find({ userId: req.userId })
    ...
    const aiRecommendations = await llmService.generateRecommendations(inventory);
    ...
    const scoreExplanation = await llmService.explainSecurityScore({
      software: latestScan.software || [],
      systemInfo: latestScan.systemInfo || {},
      secureScore: latestScan.secureScore || 50,
      totalVulnerabilities: latestScan.vulnerabilities?.total || 0,
...
function getDefenderActionSteps(id: string): string {
...
function getDefenderWhyItMatters(id: string): string {
...
function getDefenderRiskReduction(id: string): number {
...
function getDefenderPriority(id: string): 'high' | 'medium' | 'low' {
...
function getDefenderTimeEstimate(id: string): number {
