...
router.get('/', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const scheduledScans = await scheduledScanService.getScheduledScans(
      req.userId!,
      deviceId as string
    );

    // Group by device and scan type for easier frontend consumption
    const groupedScans: any = {};
...
router.post('/', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Create or update scheduled scan
    const scheduledScan = await scheduledScanService.createOrUpdateScheduledScan(
      req.userId!,
      req.user.email,
      deviceId,
      scanType,
...
router.put('/:scanId/status', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
...
router.delete('/:scanId', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { scanId } = req.params;

    const scheduledScan = await ScheduledScan.findOneAndDelete({
      _id: scanId,
      userId: req.userId
    });
...
router.post('/:scanId/run-now', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { scanId } = req.params;
    ...
    await scheduledScanService.executeScheduledScan(scheduledScan);
...
router.get('/status', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const status = scheduledScanService.getStatus();
    
    // Get summary statistics
    const totalScheduled = await ScheduledScan.countDocuments({ userId: req.userId });
    const enabledScheduled = await ScheduledScan.countDocuments({ 
      userId: req.userId, 
      enabled: true 
    });
    
    // Get next upcoming scan
    const nextScan = await ScheduledScan.findOne({
      userId: req.userId,
      enabled: true
    }).sort({ nextRun: 1 });
...
router.get('/history', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const scheduledScans = await ScheduledScan.find(query);
