...
exports.getApplicableRecommendations = getApplicableRecommendations;
exports.prioritizeRecommendations = prioritizeRecommendations;
exports.SECURITY_RECOMMENDATIONS = [
    // Email Security Recommendations
    {
        ...
        applicableWhen: (scanData) => scanData.hasEmailClient || scanData.vulnerabilities?.items?.some((v) => v.description?.toLowerCase().includes('phishing'))
    },
    ...
    // Office Security
    {
        ...
        applicableWhen: (scanData) => scanData.software?.some((s) => s.name?.toLowerCase().includes('office') ||
    ...
    // Email Client Protection
    {
        ...
        applicableWhen: (scanData) => scanData.software?.some((s) => s.name?.toLowerCase().includes('outlook') ||
    ...
    // Adobe Reader Security
    {
        ...
        applicableWhen: (scanData) => scanData.software?.some((s) => s.name?.toLowerCase().includes('adobe') &&
    ...
    // Software Update Recommendations
    {
        ...
        applicableWhen: (scanData) => {
            const chrome = scanData.software?.find((s) => s.name?.toLowerCase().includes('chrome'));
            if (!chrome)
                return false;
    ...
    },
    {
        ...
        applicableWhen: (scanData) => {
            const firefox = scanData.software?.find((s) => s.name?.toLowerCase().includes('firefox'));
            if (!firefox)
                return false;
    ...
    },
    {
        ...
        applicableWhen: (scanData) => {
            const nodejs = scanData.software?.find((s) => s.name?.toLowerCase().includes('node'));
            if (!nodejs)
                return false;
    ...
    // System Updates
    {
        ...
        applicableWhen: (scanData) => {
            if (!scanData.systemInfo?.osName?.toLowerCase().includes('windows'))
                return false;
...
function getApplicableRecommendations(scanData) {
    return exports.SECURITY_RECOMMENDATIONS.filter(rec => rec.applicableWhen(scanData));
}
