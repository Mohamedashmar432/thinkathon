...
exports.getTopRemediationActivities = getTopRemediationActivities;
function getEstimatedTime(software) {
    const timeMap = {
        'chrome': '5 minutes',
...
function getUpdateSteps(software) {
    const lower = software.toLowerCase();
...
async function getTopRemediationActivities(scans, limit = 5) {
    ...
    scans.forEach(scan => {
        scan.vulnerabilities.items.forEach(vuln => {
            ...
            softwareVulns[key].maxCVSS = Math.max(softwareVulns[key].maxCVSS, vuln.cvssScore);
        });
    });
    // Convert to activities and sort by priority
    const activities = Object.values(softwareVulns)
        .map((sv, index) => ({
        priority: index + 1,
        ...
        if (b.maxCVSS !== a.maxCVSS)
            return b.maxCVSS - a.maxCVSS;
        if (b.cveIds.length !== a.cveIds.length)
            return b.cveIds.length - a.cveIds.length;
