...
function isOrganizationUser(email) {
...
router.get('/score', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (userScans.length > 0) {
            const calculatedScore = (0, scoreCalculator_1.calculateUserSecureScore)(userScans, user);
        ...
        for (const orgUser of orgUsers) {
            ...
            if (scans.length > 0) {
                const calculatedScore = (0, scoreCalculator_1.calculateUserSecureScore)(scans, orgUser);
        ...
        const totalDevices = await Scan_1.default.distinct('deviceId', {
...
function generateScoreExplanation(orgScore, userScore, contribution) {
